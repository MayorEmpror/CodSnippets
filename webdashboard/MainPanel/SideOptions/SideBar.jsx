...
const values = [
  {
    elem: <GiMountaintop className="h-1/3 w-1/3 " />,
    name: "About",
  },
  {
...
const classes =
  " text-white backdrop-blur-xl  backdrop-grayscale   h-20 m-2  opacity-75   flex flex-col justify-center items-center transition ease-in duration-300 hover:bg-slate-100/30 p-2";

export default function SideBar(props) {
  const [visible, setVisible] = React.useState(false);
  const ClassCover = /*overflow hidden removed*/ ` w-${
    props.mobile ? "1/6" : "full"
  } h-screen bg-[url('../images/Photographs/forestRoad.jpg')] shadow-2xl  bg-contain flex flex-col  classCov sepia
      ...
      {props.mobile ? (
        ...
        >
          <div className={tabContainerStyle}>
            ...
            <button className="container  font-serif">
              {values.map((v, k) => (
                <Link
                  to={"/" + v.name}
                  key={"Sidetab-" + k}
